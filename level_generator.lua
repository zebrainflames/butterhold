...
function LevelGenerator.find_valid_keys(constraints, all_templates)
	...
	for key, _ in pairs(all_templates) do
		...
		local is_valid = true
		if constraints.up and not key:find("U") then
			is_valid = false
		end
		if constraints.down and not key:find("D") then
			is_valid = false
		end
		if constraints.left and not key:find("L") then
			is_valid = false
		end
		if constraints.right and not key:find("R") then
			is_valid = false
		end
